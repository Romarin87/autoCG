...
def get_adj_matrix_from_distance(molecule, coeff=1.10):
    ...
    adj1 = np.where(distance_matrix < criteria_matrix, 1, 0)
    adj2 = np.where(distance_matrix < 1.0, 1, 0)
...
def is_same_connectivity(
    original_molecule, new_molecule, max_coeff=1.3, min_coeff=0.95, space=0.05
):
    coeff = min_coeff
    is_same = False
