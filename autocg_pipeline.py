...
def _load_atom_map_orders(save_dir: str) -> Tuple[Optional[Dict[int, int]], Optional[Dict[int, int]]]:
    """
    Returns map_r and map_r_reduced (if present) from info.pkl.
    Supports both current single-dict info.pkl and legacy three-pickle layout.
    """
    ...
    with open(info_path, "rb") as f:
        first = pickle.load(f)
        if isinstance(first, dict) and ("map_r" in first or "map_r_reduced" in first):
            reaction_info = first
        else:
            # legacy layout: reaction_info, mapping_info, matching_results
            reaction_info = first
            try:
                _ = pickle.load(f)  # mapping_info
                _ = pickle.load(f)  # matching_results
            except Exception:
                pass
    map_r = reaction_info.get("map_r")
    map_r_reduced = reaction_info.get("map_r_reduced")
    return map_r, map_r_reduced


def _reorder_coords(coords: np.ndarray, map_r: Optional[Dict[int, int]], map_r_reduced: Optional[Dict[int, int]]) -> np.ndarray:
    """
    Reorders coordinates to atom-map order. If map_r_reduced is smaller (spectators removed),
    pad missing map numbers with NaNs to keep original map_r length.
    """
    if not map_r:
        return coords
    coords = np.asarray(coords, dtype=float)
    ordered = []
    for map_num in sorted(map_r):
        if map_r_reduced and map_num not in map_r_reduced:
            ordered.append([np.nan, np.nan, np.nan])
            continue
        idx = (map_r_reduced or map_r)[map_num]
        # idx = map_r_reduced[map_num]
        if idx >= len(coords):
            raise ValueError(f"Map index {idx} out of bounds for coords with shape {coords.shape}")
        ordered.append(coords[idx])
...
def _process_entry(
    ...
    map_r, map_r_reduced = _load_atom_map_orders(save_dir)
    react_geo = _reorder_coords(_read_xyz(r_xyz), map_r, map_r_reduced)
    prod_geo = _reorder_coords(_read_xyz(p_xyz), map_r, map_r_reduced)
